...
type SourceDetection = { kind: "local"; rootDir: string } | { kind: "github" } | { kind: "unknown" };

/**
 * Statically inspects cimisy.config.ts's `source:` expression rather than
 * executing the (arbitrary TS) config file — this milestone only supports
 * localSource targets, so a syntactic check for `localSource({ rootDir })`
 * vs `githubSource(...)` is enough, and avoids needing a TS runtime loader
 * inside the CLI.
 */
function detectSource(configText: string, configFilePath: string): SourceDetection {
