...
export type Action = "read" | "write" | "publish" | "manageSchema";
...
/**
 * Used whenever a config doesn't specify `roles`/`roleMapping` — a working,
 * least-privilege RBAC setup out of the box, matching the "no team should
 * have to hand-roll access control" goal. GitHub's five collaborator
 * permission levels collapse to three cimisy roles: admin/maintain write
 * directly, write-level collaborators get PR-gated drafts, everyone else
 * is read-only.
 */
...
export interface CimisyConfig {
  ...
  /** Maps a GitHub collaborator permission level (admin/maintain/write/triage/read) to a role name above. Defaults to DEFAULT_ROLE_MAPPING if omitted. */
