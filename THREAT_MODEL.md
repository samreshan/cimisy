...
- **No remote value reaches a command line.** The App install URL embeds a slug from GitHub's manifest-conversion response and is handed to a process launcher. CodeQL flagged this on the v2.5 branch and the flag was right: the Windows path went through `cmd /c start`, which parses its own command line, so a shell metacharacter surviving GitHub's slugification would have been command injection — precisely the "trusted third party, but not blindly" case. Now the slug is percent-encoded, the URL is checked against an allowlist of exactly the two origins the wizard ever opens (`https://github.com` and its own `http://127.0.0.1` server) before any process is spawned, and Windows uses `rundll32 url.dll,FileProtocolHandler`, which takes the URL as a plain argv element. `spawn` is never given `shell: true` anywhere in the CLI. Verified by `src/cli/__tests__/setup-github.test.ts`'s `isOpenableUrl` cases (foreign hosts, look-alike subdomains, `file:`/`javascript:`/`data:` schemes, non-loopback http).
